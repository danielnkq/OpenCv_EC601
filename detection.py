...
def TemplateMatching(src, temp, stepsize): # src: source image, temp: template image, stepsize: the step size for sliding the template
    ...
    for i in np.arange(0, src.shape[0] - temp.shape[0], stepsize):
        for x in np.arange(0, src.shape[1] - temp.shape[1], stepsize):
            mean_s = 0;
            var_s = 0;
            corr = 0;
            # Calculate the mean and variance of source image pixel values inside window
            # ------------------ Put your code below ------------------
            window = src[i:i + temp.shape[0], x:x + temp.shape[1]]
...
# ------------------ Put your code below ------------------ 
match_img = cv2.rectangle(source, (location[1], location[0]), (location[1] + temp.shape[1], location[0] + temp.shape[0]), (0, 0, 255), 3)
